...
title Tool ho tro kich hoat Windows - Office
color 0F
...
cls
echo.
echo ----------- Kich hoat Ban quyen Microsoft  ------------
echo -----------       DANG QUANG        ------------
echo -------------------------------------------------------
echo    [1] Kiem tra trang thai kich hoat Windows - Office
echo    [2] Go sach key Office cu
echo    [3] Kich hoat ban quyen bang key da Copy
echo    [4] Nhap va kich hoat Key (lay IID - CID)
echo    [5] Tai MAS de kich hoat Windows - Office
echo    [6] Sao luu ban quyen Windows - Office
echo    [7] Khoi phuc ban quyen da Sao luu
echo    [8] Thoat
echo -------------------------------------------------------
CHOICE /c 1234567 /n /m " Nhap lua chon cua ban:"
if %ERRORLEVEL%==1 Goto thong_tin
if %ERRORLEVEL%==2 Goto gokey
if %ERRORLEVEL%==3 Goto kich_key_copy
if %ERRORLEVEL%==4 Goto nhap_key
if %ERRORLEVEL%==5 Goto mas
if %ERRORLEVEL%==6 Goto sao_luu
if %ERRORLEVEL%==7 Goto khoi_phuc
if %ERRORLEVEL%==8 goto thoat


:kich_key_copy
cls
Echo ... Getting Key From Clipboard ...
...
:6_4_loop1
set "uninstallkey="
set /p "uninstallkey=Uninstall Product Key (Enter Nothing To Exit):"
...
:mas
cls
Echo Download - Extract Rar File - Run MAS_AIO.cmd ...
Start "" "https://github.com/massgravel/Microsoft-Activation-Scripts/archive/refs/heads/master.zip"
SETLOCAL EnableDelayedExpansion
for /F "tokens=1,2 delims=#" %%a in ('"prompt #$H#$E# & echo on & for %%b in (1) do rem"') do (set "DEL=%%a")
pause
goto kichhoat
